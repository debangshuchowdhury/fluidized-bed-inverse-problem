...
weights = np.array([0.45, 0.5, 0.0001]) #S1 / output_var  # [correlation, bed height, pressure]

def weighted_mse(y_true, y_pred):

    bheight_pred = y_pred[:,1] * std_y[1] + mean_y[1]
    e = 0.7166
    correlation = (1 - e) * (2730 - 1.165) * 9.81 * bheight_pred
    ...
    return weights[0] * loss_pressure + weights[1] * loss_height + loss_extra * weights[2]
...
#     extended_range = np.linspace(max_flow_original, extended_max, n_extended + 1)[1:]  # Exclude first point to avoid duplication
